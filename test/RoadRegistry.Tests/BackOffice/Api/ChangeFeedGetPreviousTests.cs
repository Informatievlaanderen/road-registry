    ...
    public class ChangeFeedGetPreviousTests
    {
        ...
        public async Task When_downloading_previous_changes_without_specifying_a_before_entry()
        {
            ...
            using (var context = await _fixture.CreateEmptyEditorContextAsync(await _fixture.CreateDatabaseAsync()))
            {
                ...
                {
                    exception.Errors.ShouldCompare(new List<ValidationFailure> { new ValidationFailure("BeforeEntry", "BeforeEntry query string parameter is missing.") });
                }
            }
        }
        ...
        public async Task When_downloading_previous_changes_with_too_many_before_entries_specified()
        {
            ...
            using (var context = await _fixture.CreateEmptyEditorContextAsync(await _fixture.CreateDatabaseAsync()))
            {
                ...
                {
                    exception.Errors.ShouldCompare(new List<ValidationFailure> { new ValidationFailure("BeforeEntry", "BeforeEntry query string parameter requires exactly 1 value.") });
                }
            }
        }
        ...
        public async Task When_downloading_previous_changes_with_a_before_entry_that_is_not_an_integer()
        {
            ...
            using (var context = await _fixture.CreateEmptyEditorContextAsync(await _fixture.CreateDatabaseAsync()))
            {
                ...
                {
                    exception.Errors.ShouldCompare(new List<ValidationFailure> { new ValidationFailure("BeforeEntry", "BeforeEntry query string parameter value must be an integer.") });
                }
            }
        }
