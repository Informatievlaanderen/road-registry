namespace RoadRegistry.Tests.AggregateTests.RoadSegment.RemoveRoadSegmentBecauseOfMigration;
...
public class AggregateTests : AggregateTestBase
{
    ...
    public void ThenRoadSegmentRemoved()
    {
        ...
        // Act
        var problems = segment.RemoveBecauseOfMigration(Fixture.Create<Provenance>());

        // Assert
        problems.Should().HaveNoError();
        segment.GetChanges().Should().HaveCount(1);

        var segmentRemoved = (RoadSegmentWasRemovedBecauseOfMigration)segment.GetChanges().Single();
        segmentRemoved.RoadSegmentId.Should().Be(segment.RoadSegmentId);
        segmentRemoved.StartNodeId.Should().Be(segment.StartNodeId);
        segmentRemoved.EndNodeId.Should().Be(segment.EndNodeId);
        segmentRemoved.Geometry.Should().Be(segment.Geometry);
    }
    ...
    public void StateCheck()
    {
        ...
        var evt = Fixture.Create<RoadSegmentWasRemovedBecauseOfMigration>();
        ...
        segment.Status.Should().Be(segmentAdded.Status);
