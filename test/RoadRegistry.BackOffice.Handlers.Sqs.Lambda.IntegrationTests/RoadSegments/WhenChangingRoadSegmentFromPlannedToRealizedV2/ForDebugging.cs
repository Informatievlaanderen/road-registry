namespace RoadRegistry.BackOffice.Handlers.Sqs.Lambda.IntegrationTests.RoadSegments.WhenChangingRoadSegmentFromPlannedToRealizedV2;
...
/// <summary>
/// Runs the planned-to-realized lambda against whatever database the configuration points at, so a scenario that only
/// shows up on a real network - a planned segment that has to snap onto road nodes that are already there, add the
/// ones that are not, and turn the crossings it makes into gelijkgrondse kruisingen - can be reproduced without going
/// through the API and the queue.
///
...
public class ForDebugging
{
    ...
    public async Task WithCustomRequestOnActualServer()
    {
        ...
        // Act
        await handler.Handle(new ChangeRoadSegmentFromPlannedToRealizedV2SqsLambdaRequest("abc", sqsRequest), CancellationToken.None);
