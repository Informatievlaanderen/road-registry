...
// The handler for a status change that hooks the segment into the network: it has to scope on the geometry,
// because a segment outside the network carries no road nodes to be found through.
public class GivenAConnectingStatusChange : BackOfficeLambdaTest
{
    ...
    public GivenAConnectingStatusChange(ITestOutputHelper testOutputHelper) : base(testOutputHelper)
    {
    }
        ...
        bool deadEndNodeIsMigrated = true)
    {
        ...
        var sqsRequest = new ChangeRoadSegmentStatusV2SqsRequest
        {
            ...
            StatusChange = RoadSegmentStatusChange.PlannedToRealized,
