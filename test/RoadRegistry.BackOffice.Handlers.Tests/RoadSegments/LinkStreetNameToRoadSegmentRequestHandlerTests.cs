...
public class LinkStreetNameToRoadSegmentRequestHandlerTests : LinkUnlinkStreetNameToFromRoadSegmentTestsBase
    <LinkStreetNameToRoadSegmentRequest, LinkStreetNameToRoadSegmentResponse, LinkStreetNameToRoadSegmentRequestHandler>
{
    public LinkStreetNameToRoadSegmentRequestHandlerTests(CommandHandlerDispatcher commandHandlerDispatcher, ILogger<LinkStreetNameToRoadSegmentRequestHandler> logger)
        : base(commandHandlerDispatcher, logger)
    {
    }

    protected override LinkStreetNameToRoadSegmentRequestHandler CreateHandler(CommandHandlerDispatcher commandHandlerDispatcher, ILogger<LinkStreetNameToRoadSegmentRequestHandler> logger)
    {
        return new LinkStreetNameToRoadSegmentRequestHandler(commandHandlerDispatcher, logger, Fixture.Store, RoadRegistryContext, StreetNameCache);
    }
    ...
    public async Task LinkStreetNameToRoadSegment_LeftStreetName_AlreadyLinked()
    {
        ...
        {
            var request = new LinkStreetNameToRoadSegmentRequest(1, StreetNamePuri(WellKnownStreetNameIds.Proposed), null);
    ...
    public async Task LinkStreetNameToRoadSegment_LeftStreetName_NotExists()
    {
        ...
        var ex = await Assert.ThrowsAsync<ValidationException>(() =>
        {
            var request = new LinkStreetNameToRoadSegmentRequest(1, StreetNamePuri(99999), null);
            return Handler.HandleAsync(request, CancellationToken.None);
        });
    ...
    public async Task LinkStreetNameToRoadSegment_LeftStreetName_Proposed_Current(int streetNameId)
    {
        ...
        var request = new LinkStreetNameToRoadSegmentRequest(1, StreetNamePuri(streetNameId), null);
    ...
    public async Task LinkStreetNameToRoadSegment_LeftStreetName_Retired()
    {
        ...
        var ex = await Assert.ThrowsAsync<ValidationException>(() =>
        {
            var request = new LinkStreetNameToRoadSegmentRequest(1, StreetNamePuri(WellKnownStreetNameIds.Retired), null);
            return Handler.HandleAsync(request, CancellationToken.None);
        });
    ...
    public async Task LinkStreetNameToRoadSegment_RightStreetName_AlreadyLinked()
    {
        ...
        var ex = await Assert.ThrowsAsync<ValidationException>(() =>
        {
            var request = new LinkStreetNameToRoadSegmentRequest(1, null, StreetNamePuri(WellKnownStreetNameIds.Proposed));
            return Handler.HandleAsync(request, CancellationToken.None);
        });
    ...
    public async Task LinkStreetNameToRoadSegment_RightStreetName_NotExists()
    {
        ...
        var ex = await Assert.ThrowsAsync<ValidationException>(() =>
        {
            var request = new LinkStreetNameToRoadSegmentRequest(1, null, StreetNamePuri(99999));
            return Handler.HandleAsync(request, CancellationToken.None);
        });
        Assert.Equal("StraatnaamNietGekend", ex.Errors.Single().ErrorCode);
    }
    ...
    public async Task LinkStreetNameToRoadSegment_RightStreetName_Proposed_Current(int streetNameId)
    {
        ...
        var request = new LinkStreetNameToRoadSegmentRequest(1, null, StreetNamePuri(streetNameId));
        await Handler.HandleAsync(request, CancellationToken.None);
    ...
    public async Task LinkStreetNameToRoadSegment_RightStreetName_Retired()
    {
        ...
        var ex = await Assert.ThrowsAsync<ValidationException>(() =>
        {
            var request = new LinkStreetNameToRoadSegmentRequest(1, null, StreetNamePuri(WellKnownStreetNameIds.Retired));
            return Handler.HandleAsync(request, CancellationToken.None);
        });
        Assert.Equal("WegsegmentStraatnaamNietVoorgesteldOfInGebruik", ex.Errors.Single().ErrorCode);
    }
    ...
    public async Task LinkStreetNameToRoadSegment_RoadSegment_NotExists()
    {
        ...
        {
            var request = new LinkStreetNameToRoadSegmentRequest(int.MaxValue, StreetNamePuri(99999), null);
            return Handler.HandleAsync(request, CancellationToken.None);
        });
    }
