...
public class EuropeanRoadScenarios : FeatureCompareTranslatorScenariosBase
{
    ...
    public async Task RecordsShouldBeUnique()
    {
        ...
        Assert.Equal(expectedTranslatedProblemMessage, FileProblemTranslator.DomainV1.Translate(problem.Translate()).Message);
    }
    ...
    public async Task UnknownRoadSegmentShouldGiveProblem()
    {
        var (zipArchive, expected) = new ExtractV1ZipArchiveBuilder()
            .WithChange((builder, context) =>
            {
                ...
                builder.TestData.RoadSegment1EuropeanRoadDbaseRecord1.WS_OIDN.Value = -1;
            })
