...
namespace RoadRegistry.BackOffice.MessagingHost.Kafka.Tests
{
    ...
    public class WhenKafkaJsonMessage
    {
        ...
        {
            var message = new StreetNameWasRegistered("TEST_STREETNAME", "TEST_MUNICIPALITY", "DUMMY_NISCODE", new(DateTime.UtcNow.ToString(), "KAFKA TEST", "", "Digitaal Vlaanderen", "Testing serialization"));
            var serializedMessage = JsonConvert.SerializeObject(message, _serializerSettings);

            _outputHelper.WriteLine(serializedMessage);

            var kafkaJsonMessage = new KafkaJsonMessage(message.GetType().FullName, serializedMessage);
