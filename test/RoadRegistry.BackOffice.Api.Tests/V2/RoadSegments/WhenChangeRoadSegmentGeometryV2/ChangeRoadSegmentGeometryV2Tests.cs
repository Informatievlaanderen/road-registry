namespace RoadRegistry.BackOffice.Api.Tests.V2.RoadSegments.WhenChangeRoadSegmentGeometryV2;
...
{
    // A 10m line in Lambert 2008, so every attribute below runs from 0 to 10.
    ...
    public ChangeRoadSegmentGeometryV2Tests()
    {
        _mediator
            .Setup(x => x.Send(It.IsAny<ChangeRoadSegmentGeometryV2SqsRequest>(), It.IsAny<CancellationToken>()))
    ...
    private Task<IActionResult> Act(int id, ChangeRoadSegmentGeometryV2Parameters parameters = null)
    {
        return _controller.ChangeRoadSegmentGeometryV2(
    ...
    private static ChangeRoadSegmentGeometryV2Parameters ValidParameters()
    {
        return new ChangeRoadSegmentGeometryV2Parameters
        {
            ...
            Morfologie =
            [
                new MorfologieParameters { VanPositie = 0, TotPositie = GeometryLength, Morfologie = RoadSegmentMorphologyV2.Parallelweg.ToDutchString() }
            ],
            Wegverharding =
            [
                new WegverhardingParameters { VanPositie = 0, TotPositie = GeometryLength, Wegverharding = RoadSegmentSurfaceTypeV2.Verhard.ToDutchString() }
            ],
            Toegang =
            [
                new ToegangParameters { VanPositie = 0, TotPositie = GeometryLength, Toegang = RoadSegmentAccessRestrictionV2.OpenbareWeg.ToDutchString() }
            ],
            Straatnaam =
            [
                new StraatnaamParameters
                {
                    ...
                    VanPositie = 0,
                    TotPositie = GeometryLength,
                    Identificator = "79632"
                }
            ],
            Wegbeheerder =
            [
                new WegbeheerderParameters { Kant = RoadSegmentAttributeSide.Beide.ToDutchString(), VanPositie = 0, TotPositie = GeometryLength, Wegbeheerder = "AWV114" }
            ],
            Wegcategorie =
            [
                new WegcategorieParameters { VanPositie = 0, TotPositie = GeometryLength, Wegcategorie = RoadSegmentCategoryV2.LokaleOntsluitingsweg.ToDutchString() }
            ],
            VerkeerstypeAuto =
            [
                new VerkeerstypeParameters { VanPositie = 0, TotPositie = GeometryLength, Richting = RoadSegmentTrafficDirection.Forward.ToDutchString() }
            ],
            VerkeerstypeFiets =
            [
                new VerkeerstypeParameters { VanPositie = 0, TotPositie = GeometryLength, Richting = RoadSegmentTrafficDirection.Both.ToDutchString() }
            ],
            VerkeerstypeVoetganger =
            [
                new VerkeerstypeVoetgangerParameters { VanPositie = 0, TotPositie = GeometryLength, Richting = RoadSegmentPedestrianTrafficDirection.Both.ToDutchString() }
            ]
        };
    }
    ...
    private ChangeRoadSegmentGeometryV2SqsRequest _capturedSqsRequest;

    private void CaptureSqsRequest()
    {
        _mediator
            .Setup(x => x.Send(It.IsAny<ChangeRoadSegmentGeometryV2SqsRequest>(), It.IsAny<CancellationToken>()))
            .Callback<IRequest<LocationResult>, CancellationToken>((r, _) => _capturedSqsRequest = (ChangeRoadSegmentGeometryV2SqsRequest)r)
            .ReturnsAsync(Fixture.Create<LocationResult>());
    }

    private void VerifyNothingWasQueued()
    {
        _mediator.Verify(x => x.Send(It.IsAny<ChangeRoadSegmentGeometryV2SqsRequest>(), It.IsAny<CancellationToken>()), Times.Never);
    }
    ...
    public async Task GivenValidRequest_ThenAccepted()
    {
        ...
        _mediator.Verify(x => x.Send(It.IsAny<ChangeRoadSegmentGeometryV2SqsRequest>(), It.IsAny<CancellationToken>()), Times.Once);
    }
    ...
    [Fact]
    public async Task GivenAnInvalidRequestBodyForANonExistingRoadSegment_ThenValidationException()
    {
        // The body is validated before the road segment is looked up, so a request that is malformed *and* points at
        // a segment that is not there reports what is wrong with the request rather than a 404.
        var parameters = ValidParameters();
        parameters.WegsegmentGeometrie = null;

        await Assert.ThrowsAsync<ValidationException>(() => Act(999, parameters));
        VerifyNothingWasQueued();
    }
    ...
    [Fact]
    public async Task GivenValidRequest_ThenGeometryAndAttributesAreSentAlong()
    {
        var id = SeedRoadSegment();
        CaptureSqsRequest();
        ...
        _capturedSqsRequest.Geometry.SRID.Should().Be(WellknownSrids.Lambert08);
        _capturedSqsRequest.Geometry.Value.Length.Should().BeApproximately(GeometryLength, 0.01);

        // Every mandatory attribute has to reach the domain; none of them may be dropped on the way.
        _capturedSqsRequest.Morphology.Should().ContainSingle();
        _capturedSqsRequest.SurfaceType.Should().ContainSingle();
        _capturedSqsRequest.AccessRestriction.Should().ContainSingle();
        _capturedSqsRequest.Category.Should().ContainSingle();
        _capturedSqsRequest.StreetName.Should().ContainSingle();
        _capturedSqsRequest.MaintenanceAuthority.Should().ContainSingle();
        _capturedSqsRequest.CarTrafficDirection.Should().ContainSingle();
        _capturedSqsRequest.BikeTrafficDirection.Should().ContainSingle();
        _capturedSqsRequest.PedestrianTrafficDirection.Should().ContainSingle();
    }
