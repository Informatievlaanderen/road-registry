...
internal static class PbsProjectionEventCoverage
{
    ...
    [
        ...
        typeof(global::RoadRegistry.GradeJunction.Events.V2.GradeJunctionWasAddedBecauseOfGradeSeparatedJunctionChange), typeof(global::RoadRegistry.GradeSeparatedJunction.Events.V2.GradeSeparatedJunctionWasChangedToGradeJunction),
    ...
    public static void AssertHandledExactlyOnce(RunnerDbContextRoadNetworkChangesProjection<PbsContext> projection, params Type[] excludeEventTypes)
    {
        ...
        if (unlisted.Any())
        {
            Assert.Fail($"New domain event(s) not accounted for in {nameof(PbsProjectionEventCoverage)}.{nameof(AllRoadNetworkEvents)}:{Environment.NewLine}{string.Join(Environment.NewLine, unlisted.Select(x => x.FullName).OrderBy(x => x))}");
        }
