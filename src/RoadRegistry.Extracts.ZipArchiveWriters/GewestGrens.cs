...
{
    private static readonly IReadOnlyCollection<LineString> BorderGeometries = LoadBorderGeometries();

    private static IReadOnlyCollection<LineString> LoadBorderGeometries()
    {
        ...
        return ((MultiPolygon)shpFileReader.ReadAll()[0]).Geometries.OfType<Polygon>().Select(x => x.EnsureLambert08().ExteriorRing).ToArray();
    }
