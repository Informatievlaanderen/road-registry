...
function formatFileSize(size) {
  ...
  const newSize = Number(size) * threshold;
  const i = newSize === 0 ? 0 : Math.floor(Math.log(newSize) / Math.log(threshold));
  const fixed = (newSize / (threshold ** i)).toFixed(2);
  return `${fixed * 1} ${units[i]}`;
}

function isDefinedGlobally() {
