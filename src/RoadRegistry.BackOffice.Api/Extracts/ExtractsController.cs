namespace RoadRegistry.BackOffice.Api.Extracts
{
    ...
    public class ExtractsController : ControllerBase
    {
        ...
        {
            Abstractions.Extracts.DownloadExtractRequest request = new(body.RequestId, body.Contour);
        ...
        {
            Abstractions.Extracts.DownloadExtractByContourRequest request = new(body.Contour, body.Buffer, body.Description);
            var response = await _mediator.Send(request, cancellationToken);
            return Accepted(response);
        }
        ...
        {
            Abstractions.Extracts.DownloadExtractByNisCodeRequest request = new(body.NisCode, body.Buffer, body.Description);
            try
            {
                var response = await _mediator.Send(request, cancellationToken);
                return Accepted(response);
            ...
            CancellationToken cancellationToken)
        {
            try
            {
                Abstractions.Extracts.DownloadFileContentRequest request = new(downloadId, options.DefaultRetryAfter, options.RetryAfterAverageWindowInDays);
                var response = await _mediator.Send(request, cancellationToken);
            ...
            CancellationToken cancellationToken)
        {
            ...
            catch (UploadExtractNotFoundException exception)
            {
                ...
                {
                    Response.Headers.Add("Retry-After", exception.RetryAfterSeconds.ToString(CultureInfo.InvariantCulture));
                }
                return NotFound();
            }
        }
