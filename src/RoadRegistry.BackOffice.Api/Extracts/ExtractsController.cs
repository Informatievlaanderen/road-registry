...
public class ExtractsController : ControllerBase
{
        ...
        EditorContext editorContext)
    {
        ...
        _downloadExtractRequestBodyValidator = downloadExtractRequestBodyValidator ??
                                               throw new ArgumentNullException(
                                                   nameof(downloadExtractRequestBodyValidator));
        _downloadExtractByContourRequestBodyValidator = downloadExtractByContourRequestBodyValidator ??
                                                        throw new ArgumentNullException(
                                                            nameof(downloadExtractByContourRequestBodyValidator));
        _downloadExtractByNisCodeRequestBodyValidator = downloadExtractByNisCodeRequestBodyValidator ??
                                                        throw new ArgumentNullException(
                                                            nameof(downloadExtractByNisCodeRequestBodyValidator));
    ...
    public async Task<IActionResult> PostDownloadRequestByNisCode([FromBody] DownloadExtractByNisCodeRequestBody body)
    {
        ...
        var municipalityGeometry =
            await _editorContext.MunicipalityGeometries.SingleAsync(x => x.NisCode == body.NisCode,
        ...
        var message = new Command(
            new RequestRoadNetworkExtract
            {
                ExternalRequestId = randomExternalRequestId,
                Contour = GeometryTranslator.TranslateToRoadNetworkExtractGeometry(
                    municipalityGeometry.Geometry as MultiPolygon, body.Buffer),
                DownloadId = downloadId,
                Description = body.Description
            });
        await _dispatcher(message, HttpContext.RequestAborted);
