...
public class FileCallbackResultExamples : IExamplesProvider<FileCallbackResult>
{
    public FileCallbackResult GetExamples()
    {
        return new FileCallbackResult(
            new DownloadFileContentResponse(
                "filename.zip",
                new MediaTypeHeaderValue("application/zip"),
                (stream, cancellationToken) => Task.Delay(500, cancellationToken))
        );
    }
}