...
{
    private const string ChangeFromOutOfUseToHistorizedRoute = "{id}/acties/buitengebruiknaargehistoreerd";

    /// <summary>
    ///     Markeer een buiten gebruik zijnd wegsegment als gehistoreerd.
    /// </summary>
    ...
    [HttpPost(ChangeFromOutOfUseToHistorizedRoute, Name = nameof(ChangeRoadSegmentFromOutOfUseToHistorizedV2))]
    ...
    [SwaggerOperation(OperationId = nameof(ChangeRoadSegmentFromOutOfUseToHistorizedV2), Description = "Markeer een buiten gebruik zijnd wegsegment als gehistoreerd. Zowel de oude als de nieuwe status houden het wegsegment buiten het wegennet, dus enkel de status wijzigt: de geometrie, de attributen en de wegknopen blijven ongemoeid.")]
    public Task<IActionResult> ChangeRoadSegmentFromOutOfUseToHistorizedV2(
        [FromServices] RoadSegmentIdValidator idValidator,
        [FromRoute] int id,
        [FromServices] IDocumentStore store,
        CancellationToken cancellationToken = default)
    {
        return ChangeRoadSegmentStatusV2(RoadSegmentStatusChange.OutOfUseToHistorized, idValidator, id, store, cancellationToken);
    }
}
