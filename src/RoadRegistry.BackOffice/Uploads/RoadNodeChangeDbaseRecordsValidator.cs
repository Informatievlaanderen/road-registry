            ...
            if (moved)
                while (moved)
                {
                    ...
                    if (record != null)
                    {
                        ...
                        {
                            if (!RecordType.ByIdentifier.ContainsKey(record.RECORDTYPE.Value)) problems += recordContext.RecordTypeMismatch(record.RECORDTYPE.Value);
                        }
                        ...
                        if (record.WEGKNOOPID.HasValue)
                        {
                            ...
                            else
                            {
                                ...
                                if (RecordType.ByIdentifier.TryGetValue(record.RECORDTYPE.Value, out var recordType)) context = context.WithRoadNode(identifier, recordType);
