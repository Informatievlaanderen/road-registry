                ...
                while (moved)
                {
                    ...
                    if (record != null)
                    {
                        ...
                        else
                        {
                            ...
                            else if (!recordType.IsAnyOf(RecordType.Identical, RecordType.Added, RecordType.Modified, RecordType.Removed)) problems += recordContext.RecordTypeNotSupported(record.RECORDTYPE.Value, RecordType.Identical.Translation.Identifier, RecordType.Added.Translation.Identifier, RecordType.Modified.Translation.Identifier, RecordType.Removed.Translation.Identifier);
                        }
                        ...
                        else if (!RoadSegmentAccessRestriction.ByIdentifier.ContainsKey(record.TGBEP.Value)) problems += recordContext.RoadSegmentAccessRestrictionMismatch(record.TGBEP.Value);

                        if (!record.STATUS.HasValue)
                            problems += recordContext.RequiredFieldIsNull(record.STATUS.Field);
                        else if (!RoadSegmentStatus.ByIdentifier.ContainsKey(record.STATUS.Value)) problems += recordContext.RoadSegmentStatusMismatch(record.STATUS.Value);

                        if (!record.CATEGORIE.HasValue)
                            problems += recordContext.RequiredFieldIsNull(record.CATEGORIE.Field);
                        else if (!RoadSegmentCategory.ByIdentifier.ContainsKey(record.CATEGORIE.Value)) problems += recordContext.RoadSegmentCategoryMismatch(record.CATEGORIE.Value);

                        if (!record.METHODE.HasValue)
                            problems += recordContext.RequiredFieldIsNull(record.METHODE.Field);
                        else if (!RoadSegmentGeometryDrawMethod.ByIdentifier.ContainsKey(record.METHODE.Value)) problems += recordContext.RoadSegmentGeometryDrawMethodMismatch(record.METHODE.Value);

                        if (!record.MORFOLOGIE.HasValue)
                            problems += recordContext.RequiredFieldIsNull(record.MORFOLOGIE.Field);
                        else if (!RoadSegmentMorphology.ByIdentifier.ContainsKey(record.MORFOLOGIE.Value)) problems += recordContext.RoadSegmentMorphologyMismatch(record.MORFOLOGIE.Value);

                        if (!record.B_WK_OIDN.HasValue)
                            problems += recordContext.RequiredFieldIsNull(record.B_WK_OIDN.Field);
                        else if (!RoadNodeId.Accepts(record.B_WK_OIDN.Value)) problems += recordContext.BeginRoadNodeIdOutOfRange(record.B_WK_OIDN.Value);

                        if (!record.E_WK_OIDN.HasValue)
                            problems += recordContext.RequiredFieldIsNull(record.E_WK_OIDN.Field);
                        else if (!RoadNodeId.Accepts(record.E_WK_OIDN.Value)) problems += recordContext.EndRoadNodeIdOutOfRange(record.E_WK_OIDN.Value);

                        if (record.LSTRNMID.Value.HasValue && !CrabStreetnameId.Accepts(record.LSTRNMID.Value.Value)) problems += recordContext.LeftStreetNameIdOutOfRange(record.LSTRNMID.Value.Value);

                        if (record.RSTRNMID.Value.HasValue && !CrabStreetnameId.Accepts(record.RSTRNMID.Value.Value)) problems += recordContext.RightStreetNameIdOutOfRange(record.RSTRNMID.Value.Value);
                        ...
                        if (string.IsNullOrEmpty(record.BEHEERDER.Value)) problems += recordContext.RequiredFieldIsNull(record.BEHEERDER.Field);
                    }
