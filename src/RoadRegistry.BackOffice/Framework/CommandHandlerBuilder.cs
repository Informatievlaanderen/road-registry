...
{
    internal CommandHandlerBuilder(Action<Func<Command<TCommand>, CommandMetadata, CancellationToken, Task>> builder)
    {
        Builder = builder ??
                  throw new ArgumentNullException(nameof(builder));
    }

    public Action<Func<Command<TCommand>, CommandMetadata, CancellationToken, Task>> Builder { get; }

    public void Handle(Func<Command<TCommand>, CommandMetadata, CancellationToken, Task> handler)
    {
    ...
    private sealed class WithContextPipeline<TContext> : ICommandHandlerBuilder<TContext, TCommand>
    {
        public WithContextPipeline(
            Action<Func<Command<TCommand>, CommandMetadata, CancellationToken, Task>> builder,
            Func<
                Func<TContext, Command<TCommand>, CommandMetadata, CancellationToken, Task>,
                Func<Command<TCommand>, CommandMetadata, CancellationToken, Task>> pipeline)
        {
            Builder = builder;
            Pipeline = pipeline;
        }

        private Action<Func<Command<TCommand>, CommandMetadata, CancellationToken, Task>> Builder { get; }

        private Func<
            Func<TContext, Command<TCommand>, CommandMetadata, CancellationToken, Task>,
            Func<Command<TCommand>, CommandMetadata, CancellationToken, Task>> Pipeline { get; }

        public void Handle(Func<TContext, Command<TCommand>, CommandMetadata, CancellationToken, Task> handler)
        {
            if (handler == null) throw new ArgumentNullException(nameof(handler));
