...
public static class ProblemTranslator
{
    private static readonly Dictionary<ProblemCode, Converter<Problem, ProblemTranslation>> ProblemCodeDutchTranslators = new()
    {
        ...
        {ProblemCode.RoadSegment.MaintenanceAuthorityCode.NotValid, problem => new(problem.Severity, "WegbeheerdercodeNietGekend", $"De opgegeven wegbeheerdercode '{problem.GetParameterValue(MaintenanceAuthorityCodeNotValid.ParameterName.OrganizationId)}' komt niet overeen met een (OVO-code die correspondeert met een) code gekend door het Wegenregister.") },
