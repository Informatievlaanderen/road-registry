...
public static class FileProblemTranslator
{
    public static readonly Converter<FileProblem, ProblemTranslation> Dutch = problem =>
    {
        ...
        return problem.Reason switch
        {
            ...
            nameof(DbaseFileProblems.GradeSeparatedJunctionMissing) => translation with { Message = $"De wegsegmenten {problem.Parameters[1].Value} en {problem.Parameters[2].Value} snijden elkaar, maar er is geen ongelijkgrondse kruising met deze wegsegmenten als onder- en bovenliggend wegsegment." },
