...
public static class FileProblemTranslator
{
    public static readonly Converter<FileProblem, ProblemTranslation> Dutch = problem =>
    {
        ...
        return problem.Reason switch
        {
            ...
            nameof(DbaseFileProblems.UpperRoadSegmentIdOutOfRange) => translation with { Message = $"De {DbaseRecordLabel()} heeft een ongeldige bovenliggende wegsegment identificator in veld {nameof(GradeSeparatedJunctionChangeDbaseRecord.Schema.BO_WS_OIDN)} ({problem.GetParameterValue("Actual")})." },
            nameof(DbaseFileProblems.LowerRoadSegmentIdOutOfRange) => translation with { Message = $"De {DbaseRecordLabel()} heeft een ongeldige onderliggende wegsegment identificator in veld {nameof(GradeSeparatedJunctionChangeDbaseRecord.Schema.ON_WS_OIDN)} ({problem.GetParameterValue("Actual")})." },
            nameof(DbaseFileProblems.NumberedRoadOrdinalOutOfRange) => translation with { Message = $"De {DbaseRecordLabel()} heeft een ongeldig volgnummer in veld {nameof(NumberedRoadChangeDbaseRecord.Schema.VOLGNUMMER)} ({problem.GetParameterValue("Actual")})." },
            nameof(DbaseFileProblems.NumberedRoadDirectionMismatch) => translation with { Message = $"De {DbaseRecordLabel()} bevat een ongeldige richting in veld {nameof(NumberedRoadChangeDbaseRecord.Schema.RICHTING)} ({problem.GetParameterValue("Actual")}). Verwachte 1 van volgende waarden: {problem.GetParameterValue("ExpectedOneOf")}." },
            nameof(DbaseFileProblems.RoadNodeTypeMismatch) => translation with { Message = $"De {DbaseRecordLabel()} bevat een ongeldig type wegknoop in veld {nameof(RoadNodeChangeDbaseRecord.Schema.TYPE)} ({problem.GetParameterValue("Actual")}). Verwachte 1 van volgende waarden: {problem.GetParameterValue("ExpectedOneOf")}." },
            ...
            nameof(DbaseFileProblems.RoadSegmentAccessRestrictionMismatch) => translation with { Message = $"De {DbaseRecordLabel()} bevat een ongeldige toegangsbeperking in veld {nameof(RoadSegmentChangeDbaseRecord.Schema.TGBEP)} ({problem.GetParameterValue("Actual")}). Verwachte 1 van volgende waarden: {problem.GetParameterValue("ExpectedOneOf")}." },
            nameof(DbaseFileProblems.RoadSegmentStatusMismatch) => translation with { Message = $"De {DbaseRecordLabel()} bevat een ongeldige status in veld {nameof(RoadSegmentChangeDbaseRecord.Schema.STATUS)} ({problem.GetParameterValue("Actual")}). Verwachte 1 van volgende waarden: {problem.GetParameterValue("ExpectedOneOf")}." },
            ...
            nameof(DbaseFileProblems.RoadSegmentCategoryMismatch) => translation with { Message = $"De {DbaseRecordLabel()} bevat een ongeldige wegcategorie in veld {nameof(RoadSegmentChangeDbaseRecord.Schema.CATEGORIE)} ({problem.GetParameterValue("Actual")}). Verwachte 1 van volgende waarden: {problem.GetParameterValue("ExpectedOneOf")}." },
            nameof(DbaseFileProblems.RoadSegmentGeometryDrawMethodMismatch) => translation with { Message = $"De {DbaseRecordLabel()} bevat een ongeldige methode in veld {nameof(RoadSegmentChangeDbaseRecord.Schema.METHODE)} ({problem.GetParameterValue("Actual")}). Verwachte 1 van volgende waarden: {problem.GetParameterValue("ExpectedOneOf")}." },
            nameof(DbaseFileProblems.RoadSegmentMorphologyMismatch) => translation with { Message = $"De {DbaseRecordLabel()} bevat een ongeldige morfologie in veld {nameof(RoadSegmentChangeDbaseRecord.Schema.MORFOLOGIE)} ({problem.GetParameterValue("Actual")}). Verwachte 1 van volgende waarden: {problem.GetParameterValue("ExpectedOneOf")}." },
            nameof(DbaseFileProblems.BeginRoadNodeIdOutOfRange) => translation with { Message = $"De {DbaseRecordLabel()} heeft een ongeldige begin wegknoop in veld {nameof(RoadSegmentChangeDbaseRecord.Schema.B_WK_OIDN)} ({problem.GetParameterValue("Actual")})." },
            nameof(DbaseFileProblems.EndRoadNodeIdOutOfRange) => translation with { Message = $"De {DbaseRecordLabel()} heeft een ongeldige eind wegknoop in veld {nameof(RoadSegmentChangeDbaseRecord.Schema.E_WK_OIDN)} ({problem.GetParameterValue("Actual")})." },
            nameof(DbaseFileProblems.LeftStreetNameIdOutOfRange) => translation with { Message = $"De {DbaseRecordLabel()} heeft een ongeldige straat naam id in veld {nameof(RoadSegmentChangeDbaseRecord.Schema.LSTRNMID)} ({problem.GetParameterValue("Actual")})." },
            nameof(DbaseFileProblems.RightStreetNameIdOutOfRange) => translation with { Message = $"De {DbaseRecordLabel()} heeft een ongeldige straat naam id in veld {nameof(RoadSegmentChangeDbaseRecord.Schema.RSTRNMID)} ({problem.GetParameterValue("Actual")})." },
            nameof(DbaseFileProblems.BeginRoadNodeIdEqualsEndRoadNode) => translation with { Message = $"De {DbaseRecordLabel()} heeft een begin wegknoop in veld {nameof(RoadSegmentChangeDbaseRecord.Schema.B_WK_OIDN)} ({problem.GetParameterValue("Begin")}) die gelijk is aan de eindwegknoop in veld {nameof(RoadSegmentChangeDbaseRecord.Schema.E_WK_OIDN)} ({problem.GetParameterValue("End")})." },
            ...
            nameof(DbaseFileProblems.LaneCountOutOfRange) => translation with { Message = $"De {DbaseRecordLabel()} bevat een ongeldig rijstrook aantal in veld {nameof(RoadSegmentLaneChangeDbaseRecord.Schema.AANTAL)} ({problem.GetParameterValue("Actual")})." },
            nameof(DbaseFileProblems.LaneDirectionMismatch) => translation with { Message = $"De {DbaseRecordLabel()} bevat een ongeldige rijstrook richting in veld {nameof(RoadSegmentLaneChangeDbaseRecord.Schema.RICHTING)} ({problem.GetParameterValue("Actual")}). Verwachte 1 van volgende waarden: {problem.GetParameterValue("ExpectedOneOf")}." },
            nameof(DbaseFileProblems.SurfaceTypeMismatch) => translation with { Message = $"De {DbaseRecordLabel()} bevat een ongeldig type wegverharding in veld {nameof(RoadSegmentSurfaceChangeDbaseRecord.Schema.TYPE)} ({problem.GetParameterValue("Actual")}). Verwachte 1 van volgende waarden: {problem.GetParameterValue("ExpectedOneOf")}." },
            nameof(DbaseFileProblems.WidthOutOfRange) => translation with { Message = $"De {DbaseRecordLabel()} bevat een ongeldige wegbreedte in veld {nameof(RoadSegmentWidthChangeDbaseRecord.Schema.BREEDTE)} ({problem.GetParameterValue("Actual")})." },
