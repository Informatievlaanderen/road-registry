namespace RoadRegistry.BackOffice
{
    ...
    public static class ValidationExtensions
    {
            ...
            where T : IRoadSegmentDbaseRecord
        {
            ...
            if (record.LSTRNMID.Value.HasValue && !StreetNameLocalId.Accepts(record.LSTRNMID.Value.Value))
            {
                problems += recordContext.LeftStreetNameIdOutOfRange(record.LSTRNMID.Value.Value);
            }

            if (record.RSTRNMID.Value.HasValue && !StreetNameLocalId.Accepts(record.RSTRNMID.Value.Value))
            {
