...
public class RoadSegmentUnflattener
{
        ...
        OgcFeaturesCache ogcFeaturesCache)
    {
        ...
        var mergedGeometry = segments.Count > 1
            ? longestSegment.Attributes.Geometry.Factory.CreateMultiLineString([new LineString(segments.SelectMany(s => s.Attributes.Geometry.Coordinates).Distinct().ToArray())])
        ...
        CancellationToken cancellationToken)
    {
        ...
        while (segmentsQueue.Count > 0)
        {
            ...
            if (EnqueueSplitResult(tryFixSelfIntersectingResult))
            {
            ...
            if (EnqueueSplitResult(tryFixSameStartEndNodeResult))
            {
                result.Remove(segment);
                continue;
            }
            ...
            if (EnqueueSplitResult(tryFixMultipleIntersectionsWithOtherSegmentsResult))
            {
                result.Remove(segment);
                continue;
            }
        ...
        bool EnqueueSplitResult((IReadOnlyCollection<RoadSegmentFeatureWithDynamicAttributes> RoadSegments, ZipArchiveProblems Problems) splitResult)
        {
        ...
        VerificationContextTolerances tolerances)
    {
        ...
        return (new IntersectionGeometry(geometryWhichMustContainNode, geometry.Factory.CreatePoint(middlePoint)), ZipArchiveProblems.None);
    }
    ...
    private IntersectionGeometry? DetectSameStartEndNode(MultiLineString geometry, VerificationContextTolerances tolerances)
    {
        ...
        if (startPoint.IsReasonablyEqualTo(endPoint, tolerances))
        {
            ...
            return new IntersectionGeometry(geometryWhichMustContainNode, geometry.Factory.CreatePoint(middlePoint));
        }
        ...
        ZipArchiveEntryFeatureCompareTranslateContext context)
    {
        ...
        {
            var (intersection, problems) = GetFirstMultipleIntersectionsIntersectionGeometry(segment.Attributes.Geometry, otherSegment, context.Tolerances);
            if (intersection is null)
            {
                continue;
            }
    ...
    private (IntersectionGeometry? Intersection, ZipArchiveProblems Problems) GetFirstMultipleIntersectionsIntersectionGeometry(
        MultiLineString geometry,
        RoadSegmentFeatureWithDynamicAttributes otherSegment,
        VerificationContextTolerances tolerances)
    {
        var otherGeometry = otherSegment.Attributes.Geometry;
    ...
    private (IReadOnlyCollection<RoadSegmentFeatureWithDynamicAttributes> SplitSegments, ZipArchiveProblems Problems) TrySplitAtValidatieknopen(
        RoadSegmentFeatureWithDynamicAttributes segment,
        ...
        ZipArchiveEntryFeatureCompareTranslateContext context)
    {
        ...
        {
            //return ([], ZipArchiveProblems.Single());
            throw new NotImplementedException("//TODO-pr Problem: No knoop found within expected range");
        }
    ...
    private sealed record IntersectionGeometry(LineString GeometryWhichMustContainNode, Point IdealNodeLocation);
}
