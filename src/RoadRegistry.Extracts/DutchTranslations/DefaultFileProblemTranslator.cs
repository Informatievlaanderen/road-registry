...
public sealed class DefaultFileProblemTranslator : FileProblemTranslator
{
    ...
    protected override ProblemTranslation? InnerTranslate(FileProblem problem)
    {
        ...
        return problem.Reason switch
        {
            ...
            nameof(DbaseFileProblems.IdentifierZero) => translation with { Message = $"De {DbaseRecordLabel()} bevat een identificator die 0 is." },
            nameof(DbaseFileProblems.RequiredFieldIsNull) => translation with { Message = $"De {DbaseRecordLabel()} heeft geen waarde voor veld {problem.GetParameterValue("Field")}." },
            nameof(DbaseFileProblems.IdentifierNotUnique) => translation with { Message = $"De {DbaseRecordLabel()} bevat dezelfde {problem.GetParameterValue("IdentifierField", required: false) ?? "identifier"} {problem.GetParameterValue("Identifier")} als dbase record {problem.GetParameterValue("TakenByRecordNumber")}." },
            nameof(DbaseFileProblems.IdentifierNotUniqueButAllowed) => translation with { Message = $"De {DbaseRecordLabel()} ({problem.GetParameterValue("RecordType")}) bevat dezelfde identifier {problem.GetParameterValue("Identifier")} als dbase record {problem.GetParameterValue("TakenByRecordNumber")} ({problem.GetParameterValue("TakenByRecordType")})." },
            ...
            nameof(DbaseFileProblems.EuropeanRoadNotUnique) => translation with { Message = $"De {DbaseRecordLabel("EU_OIDN", problem.GetParameterValue("AttributeId"))} heeft hetzelfde {problem.GetParameterValue("RoadSegmentIdentifierField") ?? "WS_OIDN"} en {problem.GetParameterValue("NumberField") ?? "EUNUMMER"} als de dbase record {problem.GetParameterValue("TakenByRecordNumber")} met EU_OIDN {problem.GetParameterValue("TakenByAttributeId")}." },
            ...
            nameof(DbaseFileProblems.NationalRoadNotUnique) => translation with { Message = $"De {DbaseRecordLabel("NW_OIDN", problem.GetParameterValue("AttributeId"))} heeft hetzelfde {problem.GetParameterValue("RoadSegmentIdentifierField") ?? "WS_OIDN"} en {problem.GetParameterValue("NumberField") ?? "IDENT2"} als de dbase record {problem.GetParameterValue("TakenByRecordNumber")} met NW_OIDN {problem.GetParameterValue("TakenByAttributeId")}." },
            ...
            nameof(DbaseFileProblems.RoadSegmentMaintenanceAuthorityNotKnown) => translation with { Message = $"De {DbaseRecordLabel()} bevat een ongeldige wegbeheerder in veld {problem.GetParameterValue("Field", required: false) ?? nameof(RoadSegmentDbaseRecord.Schema.BEHEER)}. De opgegeven wegbeheerdercode ({problem.GetParameterValue(MaintenanceAuthorityNotKnown.ParameterName.OrganizationId)}) komt niet overeen met een (OVO-code die correspondeert met een) code gekend door het Wegenregister." },
            nameof(DbaseFileProblems.RoadSegmentMaintenanceAuthorityOutOfRange) => translation with { Message = $"De {DbaseRecordLabel()} bevat een ongeldig organisatie identificator in veld {problem.GetParameterValue("Field", required: false) ?? nameof(RoadSegmentDbaseRecord.Schema.BEHEER)} ({problem.GetParameterValue("Actual")})." },
