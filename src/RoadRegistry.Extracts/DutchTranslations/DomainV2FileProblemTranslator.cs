...
public sealed class DomainV2FileProblemTranslator : FileProblemTranslator
{
    ...
    protected override ProblemTranslation? InnerTranslate(FileProblem problem)
    {
        ...
        return problem.Reason switch
        {
            ...
            nameof(DbaseFileProblems.RoadSegmentIdOutOfRange) => translation with { Message = $"De {DbaseRecordLabel(nameof(RoadSegmentDbaseRecord.WS_OIDN), problem.GetParameterValue("Actual"))} heeft een ongeldige wegsegment identificator." },
            nameof(DbaseFileProblems.RoadSegmentTempIdOutOfRange) => translation with { Message = $"De {DbaseRecordLabel(nameof(RoadSegmentDbaseRecord.WS_TEMPID), problem.GetParameterValue("Actual"))} heeft een ongeldige wegsegment identificator." },
            ...
            nameof(DbaseFileProblems.RoadSegmentAccessRestrictionV2Mismatch) => translation with { Message = $"De {DbaseRecordLabel()} bevat een ongeldige toegangsbeperking in veld {nameof(RoadSegmentDbaseRecord.Schema.TOEGANG)} ({problem.GetParameterValue("Actual")}). Verwachte 1 van volgende waarden: {problem.GetParameterValue("ExpectedOneOf")}." },
            nameof(DbaseFileProblems.RoadSegmentMorphologyV2Mismatch) => translation with { Message = $"De {DbaseRecordLabel()} bevat een ongeldige morfologie in veld {nameof(RoadSegmentDbaseRecord.Schema.MORF)} ({problem.GetParameterValue("Actual")}). Verwachte 1 van volgende waarden: {problem.GetParameterValue("ExpectedOneOf")}." },
            nameof(DbaseFileProblems.RoadSegmentStatusV2Mismatch) => translation with { Message = $"De {DbaseRecordLabel()} bevat een ongeldige status in veld {nameof(RoadSegmentDbaseRecord.Schema.STATUS)} ({problem.GetParameterValue("Actual")}). Verwachte 1 van volgende waarden: {problem.GetParameterValue("ExpectedOneOf")}." },
            ...
            nameof(DbaseFileProblems.GradeSeparatedJunctionNotUnique) => translation with { Message = $"De {DbaseRecordLabel()} heeft hetzelfde onder- en bovenliggende wegsegment als de dbase record met {problem.GetParameterValue("IdentifierField")} {problem.GetParameterValue("OtherJunctionId")}" },

            _ => DomainV1.Translate(problem)
        };
