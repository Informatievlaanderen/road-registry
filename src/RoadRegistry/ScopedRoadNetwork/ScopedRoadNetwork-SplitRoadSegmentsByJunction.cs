...
public partial class ScopedRoadNetwork
{
        ...
        ILogger? logger = null)
    {
        ...
        // Each segment gets its own temporary-id range (1/2 and 3/4) because both share a single IdentifierTranslator.
        var result1 = SplitSegmentAtCut(segment1, roadSegmentId1, cut1, sharedNodeCoordinate, firstTemporaryId: 1, idGenerator, provenance, context);
        var result2 = SplitSegmentAtCut(segment2, roadSegmentId2, cut2, sharedNodeCoordinate, firstTemporaryId: 3, idGenerator, provenance, context);
        ...
        {
            problems += RemoveGradeSeparatedJunction(gradeSeparatedJunctionId, context);
        }
        ...
        ScopedRoadNetworkChangeContext context)
    {
        ...
        // The crossing can lie within 15cm of an existing vertex; that vertex is dropped so the part does not end (or
        // start) with two vertices too close to each other, while the shared node stays on the crossing.
        ...
        if (keepIdentifier)
        {
            ...
            problems += AddRoadSegment(BuildAddChange(new RoadSegmentIdReference(new RoadSegmentId(firstTemporaryId)), originalStatus, attributes, shortestPart), idGenerator, context);
            problems.ThrowIfError();
        ...
        else
        {
            ...
            problems += AddRoadSegment(BuildAddChange(new RoadSegmentIdReference(new RoadSegmentId(firstTemporaryId)), originalStatus, attributes, firstPart), idGenerator, context);
            problems.ThrowIfError();
            firstPartRoadSegmentId = context.Summary.RoadSegments.Added.Last();

            problems += AddRoadSegment(BuildAddChange(new RoadSegmentIdReference(new RoadSegmentId(firstTemporaryId + 1)), originalStatus, attributes, secondPart), idGenerator, context);
            problems.ThrowIfError();
