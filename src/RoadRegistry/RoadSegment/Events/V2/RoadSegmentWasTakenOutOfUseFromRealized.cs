...
// A 'gerealiseerd' road segment was taken out of use and now counts as 'buiten gebruik', which unhooks it from the
// network. The status it takes on is the event itself, so it is not carried as a field.
//
...
public record RoadSegmentWasTakenOutOfUseFromRealized : IRoadSegmentWasDisconnectedEvent
{
    public const string EventName = "RoadSegmentWasTakenOutOfUseFromRealized"; // BE CAREFUL CHANGING THIS!!
