...
// A 'gehistoreerd' road segment was corrected back to 'gerealiseerd' and with that knotted into the network again.
// The status the segment takes on is the event itself, so it is not carried as a field.
//
...
public record RoadSegmentWasCorrectedFromHistorizedToRealized : IRoadSegmentWasConnectedEvent
{
    public const string EventName = "RoadSegmentWasCorrectedFromHistorizedToRealized"; // BE CAREFUL CHANGING THIS!!
