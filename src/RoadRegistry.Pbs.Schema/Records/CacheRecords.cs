...
public class StreetNameCacheRecordConfiguration : IEntityTypeConfiguration<StreetNameCacheRecord>
{
    ...
    {
        b.ToTable(TableName, WellKnownSchemas.PbsSchema).HasKey(p => p.StraatnaamId);
...
// organization events. Holds ALL organizations so a segment's maintainer-id columns can be resolved to a name; the
...
public class OrganizationCacheRecordConfiguration : IEntityTypeConfiguration<OrganizationCacheRecord>
{
    ...
    {
        b.ToTable(TableName, WellKnownSchemas.PbsSchema).HasKey(p => p.OrganisatieId);
