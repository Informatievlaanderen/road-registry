...
public class WmsRoadNetworkChangesProjection : DbContextRoadNetworkChangesProjection<WmsContext>
{
    public WmsRoadNetworkChangesProjection(ConnectedProjectionHandlerResolver<WmsContext> resolver, IDbContextFactory<WmsContext> dbContextFactory)
        : base("roadregistry-wms-projectionhost", resolver, dbContextFactory)
    {
    }
}
        ...
        CancellationToken token)
    {
        ...
        if (roadSegmentRecord is not null)
        {
            if (softDelete)
            {
                ...
                {
                    roadSegmentRecord.BeginTime = LocalDateTimeTranslator.TranslateFromWhen(envelope.Data.When);
