...
public class Program
{
    ...
    private static void CreateCommandsComponentView(ViewSet views, Model model)
    {
        ...
        var commands = aggregateRoot
            .Components
            .Where(x => x.Type.Contains("RoadRegistry.Road.Commands"))
            .ToList();
    ...
    private static void CreateEventsComponentView(ViewSet views, Model model)
    {
        ...
        var events = eventStore
            .Components
            .Where(x => x.Type.Contains("RoadRegistry.Road.Events"))
            .ToList();
    ...
    private static Person CreatePersonUser(Model model)
    {
        var user = model
            .AddPerson(
                PersonUserName,
                "Een gebruiker van het RoadRegistry.");
    ...
    private static IEnumerable<EventInfo> FindAllEvents()
    {
        ...
        return events.Select(x => new EventInfo
        {
            ...
            Description = x.GetCustomAttribute<EventDescriptionAttribute>().Value,
            //Description = x.FullName.Replace("RoadRegistry.Road.Events.", string.Empty),
            Type = x,
            Properties = x.GetProperties().Select(y => y.Name).ToList()
        });
    }

    private static string FormatDescription(string description) //=> $"{description}{Environment.NewLine}{string.Join(", ", properties)}";
    {
        return $"{description}";
    }
    ...
    private static void Main()
    {
        ...
        var workspace = new Workspace("RoadRegistry", "Wegen register.")
        {
