namespace Wegenregister.Structurizr
{
    using System;
    ...
    public class Program
    {
        ...
        private static void Main()
        {
            ...
            var workspace = new Workspace("Wegenregister", "Wegen register.")
            {
            ...
            var roadRegistry = CreateSystemRoadRegistry(model);

            var api = CreateContainerApi(roadRegistry);
            var apiRunner = CreateContainerApiRunner(roadRegistry);
            var apiStore = CreateContainerApiStore(roadRegistry);
            var eventStore = CreateContainerEventStore(roadRegistry);
            var aggregateRoot = CreateContainerAggregateRoot(roadRegistry);

            user.Uses(roadRegistry, "raadpleegt", "HTTPS").AddTags(CustomTags.Https);
        ...
        private static Container CreateContainerEventStore(SoftwareSystem roadRegistry)
        {
            var eventStore = roadRegistry
                .AddContainer(
                    "Eventstore",
        ...
        private static Container CreateContainerAggregateRoot(SoftwareSystem roadRegistry)
        {
            var aggregateRoot = roadRegistry
                .AddContainer(
                    "AggregateRoot",
        ...
        private static Container CreateContainerApiStore(SoftwareSystem roadRegistry)
        {
            var apiStore = roadRegistry
                .AddContainer(
                    "Loket API gegevens",
        ...
        private static Container CreateContainerApiRunner(SoftwareSystem roadRegistry)
        {
            var apiRunner = roadRegistry
                .AddContainer(
        ...
        private static Container CreateContainerApi(SoftwareSystem roadRegistry)
        {
            var api = roadRegistry
                .AddContainer(
                    "Loket API",
        ...
        private static SoftwareSystem CreateSystemRoadRegistry(Model model)
        {
            var roadRegistry = model
                .AddSoftwareSystem(
                    Location.Internal,
                    "Wegenregister",
                    "Het wegen referentie register laat gebruikers toe alle authentieke gegevens van een weg te raadplegen.");

            roadRegistry.Id = SoftwareSystemRoadRegistryId;
        ...
        private static Person CreatePersonUser(Model model)
        {
            var user = model
                .AddPerson(
                    PersonUserName,
                    "Een gebruiker van het Wegenregister.");
        ...
        {
            var roadRegistry = model.GetSoftwareSystemWithId(SoftwareSystemRoadRegistryId);

            var apiStore = roadRegistry.GetContainerWithId(ContainerApiStoreId);
            var eventStore = roadRegistry.GetContainerWithId(ContainerEventStoreId);
        ...
        private static IEnumerable<EventInfo> FindAllEvents()
        {
            var events = typeof(DomainAssemblyMarker)
                ...
                .Assembly
                .GetExportedTypes()
                .Where(x => x.AssemblyQualifiedName.Contains("Wegenregister.Road.Events"))
                .ToList();
            ...
            return events.Select(x => new EventInfo
            {
                ...
                //Description = x.FullName.Replace("Wegenregister.Road.Events.", string.Empty),
                Type = x,
        ...
        private static IEnumerable<EventInfo> FindAllCommands()
        {
            var events = typeof(DomainAssemblyMarker)
                ...
                .Assembly
                .GetExportedTypes()
                .Where(x => x.AssemblyQualifiedName.Contains("Wegenregister.Road.Commands"))
                .ToList();
            ...
            return events.Select(x => new EventInfo
            {
                Name = x.Name,
                //Description = x.GetCustomAttribute<EventDescriptionAttribute>().Value,
                Description = x.FullName.Replace("Wegenregister.Road.Commands.", string.Empty),
                Type = x,
                Properties = x.GetProperties().Select(y => y.Name).ToList()
            });
        }
        ...
        private static void CreateContextView(ViewSet views, Model model)
        {
            ...
            var contextView = views
                .CreateSystemContextView(
                    roadRegistry,
                    "Globaal overzicht",
                    "Globaal overzicht van het wegen register.");

            contextView.Add(roadRegistry);
        ...
        private static void CreateApiContainerView(ViewSet views, Model model)
        {
            ...
            var user = model.GetPersonWithName(PersonUserName);
            var api = roadRegistry.GetContainerWithId(ContainerApiId);
            var apiStore = roadRegistry.GetContainerWithId(ContainerApiStoreId);
            var eventStore = roadRegistry.GetContainerWithId(ContainerEventStoreId);
        ...
        private static void CreateApiRunnerContainerView(ViewSet views, Model model)
        {
            ...
            var apiRunner = roadRegistry.GetContainerWithId(ContainerApiRunnerId);
            var apiStore = roadRegistry.GetContainerWithId(ContainerApiStoreId);
            var eventStore = roadRegistry.GetContainerWithId(ContainerEventStoreId);
        ...
        private static void CreateEventsComponentView(ViewSet views, Model model)
        {
            ...
            var events = eventStore
                .Components
                .Where(x => x.Type.Contains("Wegenregister.Road.Events"))
                .ToList();
        ...
        {
            var roadRegistry = model.GetSoftwareSystemWithId(SoftwareSystemRoadRegistryId);
            var aggregateRoot = roadRegistry.GetContainerWithId(ContainerAggregateRootId);

            var commands = aggregateRoot
                .Components
                .Where(x => x.Type.Contains("Wegenregister.Road.Commands"))
                .ToList();
